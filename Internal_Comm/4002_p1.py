...
def connectBLE(mac_address):
    try:
        ...
        print('Beetle with mac address (', mac_address, ')is not connected...')
...
# # '''
try:
    beetleA = Thread(target=connectBLE, args=["d0:39:72:bf:ca:c0"])
    beetleA.start()
except:
    print("failed to connect to beetle A")
# '''

# '''
try:
    beetleB = Thread(target=connectBLE, args=["d0:39:72:bf:c1:d6"])
    beetleB.start()
except:
    print("failed to connect to beetle B")
# '''

# '''
try:
    beetleC = Thread(target=connectBLE, args=["d0:39:72:bf:c1:c9"])
    beetleC.start()
except:
    print("failed to connect to beetle C")
# '''
...
while True:
    ...
    # '''
    if not beetleC.is_alive():
        try:
            beetleC = Thread(target=connectBLE, args=["c4:be:84:20:19:4F"])
            beetleC.start()
        except BTLEDisconnectError:
            pass
    # '''
