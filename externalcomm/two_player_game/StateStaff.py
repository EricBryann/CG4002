...
class StateStaff(PlayerStateBase):
    ...
    def update(self, pos_self, pos_opponent, action_self, action_opponent, action_opponent_is_valid, *hit_results):
        ...
        if action_opponent_is_valid:
            if pos_self == 4 and pos_opponent != 4:
                ...
                if action_opponent == Actions.shoot:
                    if len(hit_results) == 2 and hit_results[0] == True:
                        hp_reduction = self.bullet_hp
                    else:
                        hp_reduction = 0
                elif action_opponent == Actions.grenade:
                    if len(hit_results) == 2 and hit_results[1] == True:
        ...
        if action_self == Actions.reload and self.bullets <= 0:
            self.bullets = self.magazine_size
