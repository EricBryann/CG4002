...
# ser = serial.Serial('COM3') # for example if it is COM3
...
# if you want to generate data again, can change i to say 21, so that it will continue with ypr21.txt.
i = 101
t_end = time.time() + 20
print("Wait for 20 secs before reading data")
...
while True:
    ...
    f1 = open('train/shield/ypr_k' + str(i) + ".txt", "w")
    f2 = open("train/shield/accel_k" + str(i) + ".txt", "w")
    ser.flushInput()
    ser.flushOutput()
